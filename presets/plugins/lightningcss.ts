...
export function Lightningcss(): Plugin {
  ...
  return {
    ...
    config(config) {
      ...
      const conflictConfiguration = [postcss, modules, transformer].some((c) => !isUndefined(c));
      ...
      const disabled = hasPreprocessor || conflictConfiguration || hasPostcssConfigFile;
      if (!disabled) {
        const cssTransformer = "lightningcss";
        config.css.transformer = cssTransformer;
        let tip = `${cssTransformer} ${gray(cssTransformer)}`;

        if (isUndefined(config.build.cssMinify)) {
          config.build.cssMinify = "lightningcss";
          tip = `${cssTransformer} ${gray("(transformer + cssMinify)")}`;
        }
