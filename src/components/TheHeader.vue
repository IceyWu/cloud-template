...
<template>
  <TooltipProvider>
    ...
    >
      <RouterLink to="/" class="flex items-center gap-2.5 cursor-pointer select-none no-underline">
        <img src="/logo.svg" alt="logo" class="w-7 h-7">
        <span class="text-lg font-semibold tracking-tight text-foreground">Cloud Template</span>
      </RouterLink>
        ...
        <NavigationMenu>
          <NavigationMenuList>
                    ...
                    <NavigationMenuLink as-child>
                      ...
                      >
                        <span
                          ...
                          :class="locale === availableLocale ? 'bg-primary' : 'bg-muted-foreground/30'"
                        />
                        {{ availableLocale }}
                      </a>
                    </NavigationMenuLink>
            ...
            <NavigationMenuItem v-for="r of routes" :key="r.path">
              <RouterLink
                :to="r.path"
                :class="[navigationMenuTriggerStyle(), $route.path === r.path ? '!bg-accent !text-accent-foreground font-semibold' : '']"
              >
                {{ t(r.name) }}
              </RouterLink>
            </NavigationMenuItem>
