  ...
  <div class="flex flex-col items-center gap-10 py-16 px-4 w-full max-w-4xl mx-auto">
    <Transition name="fade" mode="out-in">
      <div v-if="!isShowBtns" class="text-center space-y-4 max-w-2xl">
        ...
        <h1
          class="base-font text-5xl sm:text-6xl lg:text-7xl font-bold tracking-tight leading-tight"
        >
        ...
        <CardContent class="pt-4 space-y-4">
          ...
          <div class="pt-1">
            <el-button
              type="primary"
              class="w-full"
              @click="ElMessage.success('Element Plus 集成成功 🎉')"
            >
              Element Plus Button
            </el-button>
          </div>
          <div v-if="Object.keys(resData).length" class="rounded-lg border bg-muted/50 p-3">
            ...
            <pre
              class="text-xs text-muted-foreground overflow-auto max-h-32"
            >{{ JSON.stringify(resData, null, 2) }}</pre>
          </div>
        </CardContent>
      </Card>
    </Transition>
    ...
    <div v-show="hasTypeonFinished" class="flex flex-wrap gap-3 justify-center">
      ...
      <Button
        ...
        variant="outline"
        as="a"
        href="https://github.com/IceyWu/cloud-template"
        target="_blank"
        class="gap-2"
      >
