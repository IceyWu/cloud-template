...
export function setToken(data: DataInfo<number>, roleName?: string) {
  ...
  expires = new Date(data.expires * 1000 + Date.now()).getTime(); // 如果后端直接设置时间戳，将此处代码改为expires = data.expires，然后把上面的DataInfo<Date>改成DataInfo<number>即可
  ...
  function setUserKey(username: string, roles: string[]) {
